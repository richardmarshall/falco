...
func Digest_hash_sha256_Validate(args []value.Value) error {
	if len(args) != 1 {
		return errors.ArgumentNotEnough(Digest_hash_sha256_Name, 1, args)
	}
	args = shared.CoerceArguments(args, Digest_hash_sha256_ArgumentTypes)
	for i := range args {
		if args[i].Type() != Digest_hash_sha256_ArgumentTypes[i] {
			return errors.TypeMismatch(Digest_hash_sha256_Name, i+1, Digest_hash_sha256_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}
