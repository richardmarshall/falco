...
func Digest_hmac_sha1_base64_Validate(args []value.Value) error {
	if len(args) != 2 {
		return errors.ArgumentNotEnough(Digest_hmac_sha1_base64_Name, 2, args)
	}
	args = shared.CoerceArguments(args, Digest_hmac_sha1_base64_ArgumentTypes)
	for i := range args {
		if args[i].Type() != Digest_hmac_sha1_base64_ArgumentTypes[i] {
			return errors.TypeMismatch(Digest_hmac_sha1_base64_Name, i+1, Digest_hmac_sha1_base64_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}

// Fastly built-in function implementation of digest.hmac_sha1_base64
// Arguments may be:
// - STRING, STRING
...
func Digest_hmac_sha1_base64(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	if err := Digest_hmac_sha1_base64_Validate(args); err != nil {
