// Code generated by __generator__/interpreter.go at once

package builtin

import (
	"github.com/ysugimoto/falco/interpreter/context"
	"github.com/ysugimoto/falco/interpreter/function/errors"
	"github.com/ysugimoto/falco/interpreter/function/shared"
	"github.com/ysugimoto/falco/interpreter/value"
)

const Math_is_infinite_Name = "math.is_infinite"

var Math_is_infinite_ArgumentTypes = []value.Type{value.FloatType}

func Math_is_infinite_Validate(args []value.Value) error {
	if len(args) != 1 {
		return errors.ArgumentNotEnough(Math_is_infinite_Name, 1, args)
	}
	args = shared.CoerceArguments(args, Math_is_infinite_ArgumentTypes)
	for i := range args {
		if args[i].Type() != Math_is_infinite_ArgumentTypes[i] {
			return errors.TypeMismatch(Math_is_infinite_Name, i+1, Math_is_infinite_ArgumentTypes[i], args[i].Type())
		}
	}
	return nil
}

// Fastly built-in function implementation of math.is_infinite
// Arguments may be:
// - FLOAT
// Reference: https://developer.fastly.com/reference/vcl/functions/floating-point-classifications/math-is-infinite/
func Math_is_infinite(ctx *context.Context, args ...value.Value) (value.Value, error) {
	// Argument validations
	if err := Math_is_infinite_Validate(args); err != nil {
		return value.Null, err
	}

	x := value.Unwrap[*value.Float](args[0])
	return &value.Boolean{Value: x.IsNegativeInf || x.IsPositiveInf}, nil
}
