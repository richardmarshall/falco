...
func Digest_time_hmac_sha1_Validate(args []value.Value) error {
	...
	args = shared.CoerceArguments(args, Digest_time_hmac_sha1_ArgumentTypes)
	for i := range args {
...
func Digest_time_hmac_sha1(ctx *context.Context, args ...value.Value) (value.Value, error) {
	...
	if err := Digest_time_hmac_sha1_Validate(args); err != nil {
...
func digest_time_hmac_sha1(baseTime time.Time, secret *value.String, interval, offset *value.Integer) (value.Value, error) {
	dec, err := base64.StdEncoding.DecodeString(secret.Value)
	if err != nil {
		return value.Null, errors.New("digest.time_hmac_sha1", "Failed to base64 decode secret string")
	}
	...
	if err != nil {
		return value.Null, errors.New("digest.time_hmac_sha1", "Failed to generate TOTP password")
	}
